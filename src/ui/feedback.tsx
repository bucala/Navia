 ...
 */
export function useDiceFeedback(state: GameViewState | null) {
  ...
  useEffect(() => {
    ...
    // A new local/AI game starts its log ids from one while this hook remains
    // mounted. Reset the cursor instead of suppressing all new dice events.
    if (latestId < seen) {
      lastLogId.current = latestId;
      setDiceEvent(null);
      return;
    ...
    if (fresh.length > 0) {
      ...
      settleTimer.current = setTimeout(() => (shown.success ? sfxSuccess() : sfxFail()), settle);
...
export function WinnerOverlay({ name, onNewGame, backToMenu = false }: {
  ...
  const { t } = useLang();
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="winner-title" className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/80">
      <img src="/art/icons/icon-trophy.svg" alt="" aria-hidden="true" className="h-20 w-20 drop-shadow-lg" />
      <p id="winner-title" className="mt-4 text-2xl font-bold text-amber-200">{t('winner_title', { player: name })}</p>
